...
import { Key, Bookmark as BookmarkIcon, Info, Trash2 } from "lucide-react";
import { clearApiKey, getApiKey, saveApiKey } from "@/hooks/useNewsApi";
...
export function SettingsPage() {
  const [info, setInfo] = useState(() => getApiKey());
  ...
  const statusBadge =
    info.status === "env" ? { text: "Active (env var)", cls: "bg-primary/15 text-primary" }
    : info.status === "local" ? { text: "Active (saved key)", cls: "bg-primary/15 text-primary" }
  ...
  return (
    <div className="p-4 md:p-8 max-w-3xl mx-auto space-y-6">
      ...
      <section className="bg-card border border-border rounded-xl p-5 md:p-6 space-y-4">
        ...
        {info.key && (
          <p className="text-xs text-muted-foreground font-mono break-all">Current: {mask(info.key)}</p>
        )}
        <div className="flex flex-col sm:flex-row gap-2">
          ...
          />
          <button onClick={save} className="px-4 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-medium hover:bg-primary/90 transition-colors">
            Save Key
          </button>
        </div>
        {info.status === "local" && (
          <button onClick={clear} className="text-xs text-muted-foreground hover:text-destructive inline-flex items-center gap-1">
